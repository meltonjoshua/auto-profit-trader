...
class DemoKrakenExchange:
    ...
    def _simulate_market_movement(self, symbol: str) -> float:
        ...
        if symbol not in self.price_trends:
            self.price_trends[symbol] = random.choice(
                [-1, 0, 1]
    ...
    async def create_market_order(self, symbol: str, side: str, amount: float) -> Dict:
        ...
        try:
            ...
            if cost > 1000:  # Large order
                ...
                execution_price *= (
                    (1 + slippage) if side.lower() == "buy" else (1 - slippage)
                )
                cost = amount * execution_price
    ...
    def get_demo_stats(self) -> Dict:
        ...
        for trade in self.trade_history:
            if trade["side"] == "sell":
                ...
                profit = trade["cost"] - (
                    trade["amount"] * self.base_prices.get(trade["symbol"], 0)
                )
                profits.append(profit)

        total_profit = sum(profits) if profits else 0
        win_rate = (
            (len([p for p in profits if p > 0]) / len(profits) * 100) if profits else 0
        )
        ...
        while time.time() < end_time and self.is_running:
            try:
                ...
                if side == "buy" and self.demo_exchange.demo_balance["GBP"] > 100:
                    ...
                    side == "sell"
                    and self.demo_exchange.demo_balance.get(symbol.split("/")[0], 0)
                    > amount
                ):
                    await self.demo_exchange.create_market_order(symbol, side, amount)
                    trade_count += 1
                ...
                if random.random() < 0.1:  # 10% chance
                    condition = random.choice(
                        ["normal", "bull_run", "bear_market", "high_volatility"]
                    )
