...
class PortfolioManager:
    ...
    def __init__(self, config_manager):
        ...
        self.daily_loss_limit = config_manager.get_section("trading").get("daily_loss_limit", 100.0)
        self.max_trades_per_day = config_manager.get_section("risk_management").get("max_trades_per_day", 50)
    ...
    async def record_trade(self, trade_data: Dict) -> bool:
        ...
        try:
            ...
            ''', (
                ...
                trade_data.get('price', 0) or trade_data.get('entry_price', 0) or trade_data.get('exit_price', 0),
                trade_data.get('cost', 0) or trade_data.get('entry_cost', 0) or trade_data.get('exit_revenue', 0),
            ...
            self.logger.info(f"📝 Trade recorded: {trade_data.get('symbol')} {trade_data.get('action')}")
            return True
    ...
    async def _update_statistics(self, trade_data: Dict):
        ...
        try:
            ...
            volume = trade_data.get('cost', 0) or trade_data.get('entry_cost', 0) or trade_data.get('exit_revenue', 0)
    ...
    async def _save_daily_stats(self):
        ...
        try:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                today, self.daily_trades, self.winning_trades, self.losing_trades,
                self.daily_profit, self.total_volume, self.largest_win, self.largest_loss
            ))
    ...
    async def get_performance_metrics(self) -> Dict:
        ...
        return {
            ...
            'remaining_daily_trades': max(0, self.max_trades_per_day - self.daily_trades),
            'remaining_daily_loss_allowance': max(0, self.daily_loss_limit + self.daily_profit)
        }
    ...
    async def check_trading_limits(self) -> Dict:
        ...
        if self.daily_trades >= self.max_trades_per_day:
            ...
            limits['reasons'].append(f"Daily trade limit reached ({self.daily_trades}/{self.max_trades_per_day})")
        
        # Check daily loss limit
        if self.daily_profit <= -self.daily_loss_limit:
            limits['can_trade'] = False
            limits['reasons'].append(f"Daily loss limit reached (${self.daily_profit:.2f}/${-self.daily_loss_limit:.2f})")
...
"""
            ...
            for trade in recent_trades[:10]:  # Show last 10 trades
                ...
                profit_str = f"${trade['profit']:.2f}" if trade['profit'] != 0 else "N/A"
...
class RiskManager:
    ...
    def __init__(self, config_manager, portfolio_manager):
        ...
        self.take_profit_percentage = self.risk_config.get("take_profit_percentage", 0.05)
        self.max_trades_per_day = self.risk_config.get("max_trades_per_day", 50)
        self.cooldown_after_loss = self.risk_config.get("cooldown_after_loss", 300)  # seconds
        
        # Track recent losses for cooldown
        self.recent_losses = []
        self.last_loss_time = None
    
    async def evaluate_trade_risk(self, trade_signal: Dict, account_balance: float) -> Dict:
        ...
        try:
            ...
            if self.last_loss_time and datetime.now() - self.last_loss_time < timedelta(seconds=self.cooldown_after_loss):
                risk_assessment['approved'] = False
                risk_assessment['warnings'].append("Cooldown period active after recent loss")
                return risk_assessment
            ...
            if metrics['win_rate'] < 40 and metrics['completed_trades'] > 10:
                ...
                risk_assessment['position_size_adjustment'] = 0.5  # Reduce position size
    ...
    async def record_loss(self, loss_amount: float):
        ...
        self.recent_losses = [loss for loss in self.recent_losses if loss['timestamp'] > cutoff]
        
        self.logger.warning(f"💸 Loss recorded: ${loss_amount:.2f} - Activating {self.cooldown_after_loss}s cooldown")
    ...
    async def emergency_shutdown_check(self) -> bool:
        ...
        try:
            ...
            if emergency_conditions:
                self.logger.critical(f"🚨 EMERGENCY SHUTDOWN TRIGGERED: {', '.join(emergency_conditions)}")
                return True
