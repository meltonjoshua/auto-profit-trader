...
class ArbitrageStrategy:
    ...
    def __init__(self, exchange_manager, config_manager):
        ...
        self.min_profit_percentage = config_manager.get_section("trading").get("target_profit_arbitrage", 0.005) * 100
        self.max_position_size = config_manager.get_section("trading").get("max_position_size", 0.02)
        ...
        try:
            ...
            if len(exchange_names) > 1:
                ...
                for symbol in all_symbols:
                    count = 0
                    for exchange_name in exchange_names:
                        exchange_symbols = await self.exchange_manager.get_trading_symbols(exchange_name)
            ...
            opportunities = await self.exchange_manager.find_arbitrage_opportunities(all_symbols)
            
            if opportunities:
                self.logger.info(f"🔍 Found {len(opportunities)} arbitrage opportunities")
                for opp in opportunities:
                    self.logger.info(f"📈 {opp['symbol']}: {opp['profit_percentage']:.3f}% profit potential")
    ...
    async def execute_opportunity(self, opportunity: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            if not buy_balance or not sell_balance:
                self.logger.warning(f"Could not get balance for arbitrage trade {symbol}")
                return None
            ...
            if trade_amount < 0.001:  # Minimum trade size
                self.logger.warning(f"Trade amount too small for {symbol}: {trade_amount}")
                return None
            
            # Execute simultaneous buy and sell orders
            self.logger.info(f"🔄 Executing arbitrage: Buy {trade_amount:.6f} {symbol} on {buy_exchange} at ${buy_price:.4f}")
            self.logger.info(f"🔄 Executing arbitrage: Sell {trade_amount:.6f} {symbol} on {sell_exchange} at ${sell_price:.4f}")
            
            # Place orders simultaneously
            buy_task = self.exchange_manager.place_order(buy_exchange, symbol, 'market', 'buy', trade_amount)
            sell_task = self.exchange_manager.place_order(sell_exchange, symbol, 'market', 'sell', trade_amount)
            ...
            trade_result = {
                ...
                'profit_percentage': (actual_profit / buy_cost) * 100 if buy_cost > 0 else 0,
                'timestamp': datetime.now(),
                'buy_order_id': buy_order.get('id'),
                'sell_order_id': sell_order.get('id')
            }
            
            self.logger.info(f"✅ Arbitrage executed: ${actual_profit:.4f} profit ({trade_result['profit_percentage']:.3f}%)")
            return trade_result
...
class MomentumStrategy:
    ...
    async def get_historical_data(self, exchange_name: str, symbol: str, timeframe: str = '1m', limit: int = 100) -> Optional[np.ndarray]:
    ...
    def generate_signal(self, indicators: Dict) -> Dict:
        ...
        if not indicators:
            return {'action': 'hold', 'confidence': 0, 'reason': 'No indicators available'}
        ...
        if buy_signals > sell_signals:
            action = 'buy'
            confidence = sum(confidence_factors[:buy_signals]) / len(confidence_factors) if confidence_factors else 0
            reason = f"Buy signals: RSI={rsi:.1f}, MACD={macd:.4f}>{macd_signal:.4f}"
        elif sell_signals > buy_signals:
            action = 'sell'
            confidence = sum(confidence_factors[:sell_signals]) / len(confidence_factors) if confidence_factors else 0
            ...
            for symbol in top_symbols:
                try:
                    ...
                    if signal['action'] != 'hold' and signal['confidence'] > 0.6:
                        ...
                        self.logger.info(f"📊 {symbol}: {signal['action'].upper()} signal "
    ...
    async def _execute_buy(self, signal: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            # Place buy order
            order = await self.exchange_manager.place_order(exchange_name, symbol, 'market', 'buy', amount)
            if not order:
                return None
            ...
            self.logger.info(f"📈 Momentum BUY: {amount:.6f} {symbol} at ${current_price:.4f}")
            return trade_result
    ...
    async def _execute_sell(self, signal: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            # Place sell order
            order = await self.exchange_manager.place_order(exchange_name, symbol, 'market', 'sell', amount)
            if not order:
                return None
            ...
            self.logger.info(f"📉 Momentum SELL: {amount:.6f} {symbol} at ${current_price:.4f} "
        ...
        for position_key, position in list(self.positions.items()):
            try:
                ...
                if current_price <= position['stop_price']:
                    self.logger.warning(f"🛑 Stop-loss triggered for {symbol} at ${current_price:.4f}")
                    actions.append({
                        'action': 'sell',
                        'symbol': symbol,
                        'exchange': exchange_name,
