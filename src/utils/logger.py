...
def log_trade(symbol: str, side: str, amount: float, price: float, profit: float = 0):
    ...
    logger.info(f"TRADE | {timestamp} | {symbol} | {side} | Amount: {amount} | Price: ${price:.4f} | Profit: ${profit:.4f}")
...
def log_performance(daily_profit: float, total_profit: float, win_rate: float):
    ...
    timestamp = datetime.now().isoformat()
    logger.info(f"PERFORMANCE | {timestamp} | Daily: ${daily_profit:.2f} | Total: ${total_profit:.2f} | Win Rate: {win_rate:.1f}%")